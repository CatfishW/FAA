...
KEY = 'FAA.ResizeMouseVerification'
...
def main():
    ...
    try:
        state = evaluate('var w=FAA.Customization.FaaSpatialWorkspace.Current;'
            ...
            'var data=new object[]{w.PersistChanges,w.EditMode,w.MenuOpen,w.SelectedId,w.GetEntry("altitude").scale,dirty.GetValue(w),UnityEngine.InputSystem.Mouse.current,null};'
            'AppDomain.CurrentDomain.SetData("'+KEY+'",data);w.PersistChanges=false;w.SetEditMode(false);w.OpenMenu();w.Select("altitude");w.SetScale("altitude",.72f);'
        ...
        setup = evaluate('var w=FAA.Customization.FaaSpatialWorkspace.Current;'
            ...
            'var button=w.ControlsCanvas.GetComponentsInChildren<UnityEngine.UI.Button>().Single(b=>b.name=="Larger");'
        ...
        (ROOT/'artifacts/hud-layout-fixes/actual-mouse-click.json').write_text(json.dumps(report,indent=2)+'\n')
        ...
        if started:
            evaluate('var context=(object[])AppDomain.CurrentDomain.GetData("'+KEY+'");'
                ...
                'var w=FAA.Customization.FaaSpatialWorkspace.Current;w.SetScale("altitude",(float)context[4]);'
                ...
                'w.PersistChanges=(bool)context[0];AppDomain.CurrentDomain.SetData("'+KEY+'",null);return "Original input and layout restored";')
